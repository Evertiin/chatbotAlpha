      ...
      await Promise.all(
        medias.map(async (media: Express.Multer.File | any) => {
          ...
          if (!messageCreated.scheduleDate) {
            await queueValidation(ticket.whatsappId, tenantId, [messageCreated]);
          }
