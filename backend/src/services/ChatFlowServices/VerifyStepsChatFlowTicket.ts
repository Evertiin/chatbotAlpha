...
const isQueueDefine = async (
  ...
  if (stepCondition.action === 1) {
    ...
    if (queue.from_ia) {
      await ticket.update({ is_chat_ia: true, queueId: stepCondition.queueId })
    }
    await VerifyMensageOpenIAQueue(ticket, queue, true, true);
  ...
  if (
    ...
    if (ticket.chatFlowId) {
      ...
      const stepCondition = step.conditions.find((conditions: any) => {
        ...
        const message = String(body).toLowerCase().trim();
        return newConditions.includes(message);
      });

      if (
        !ticket.isCreated &&
        (await isAnswerCloseTicket(flowConfig, ticket, body))
      )
        return;
