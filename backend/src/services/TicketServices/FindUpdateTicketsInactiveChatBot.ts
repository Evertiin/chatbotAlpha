  ...
  import DefinedUserBotService from "../../services/ChatFlowServices/DefinedUserBotService";
    ...
    Promise.all(
      tickets.map(async (item: any) => {
        ...
        if (ticket) {
          ...
          // Antes da atualização do ticket
          try {
            const chatFlow = await ChatFlow.findOne({
              where: {
                tenantId: ticket.tenantId,
                id: ticket.chatFlowId
              },
            });
            const flowConfig = chatFlow.flow;
            const autoDistributeTickets = flowConfig?.nodeList?.[1]?.configurations?.autoDistributeTickets;
            await ticket.update(values);
            if (autoDistributeTickets) {
              ...
              console.log('autoDistributeTickets não configurado')
            }
          } catch (error) {
