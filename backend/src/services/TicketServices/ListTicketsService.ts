...
const ListTicketsService = async ({
  ...
  if (user!.profile === "admin") {
    queues = await Queue.findAll({
      ...
      attributes: ["id" as "queueId"]
    });

    queues = queues.map(q => { return { queueId: q.id } })
  ...
  queuesIdsUser.push(0)
  // check is queues filter and verify access user queue
  if (queuesIds) {
    const newArray: number[] = [];
    queuesIds.forEach(i => {
      const idx = queuesIdsUser.findIndex(item => Number(i) == item);
      if (idx > -1) {
  ...
  if (profile == 'admin') {
    ...
    u."name" as username,
    q.queue,
    jsonb_build_object('id', w.id, 'name', w."name" , 'isDeleted' , w."isDeleted") whatsapp,
    t.*
    from "Tickets" t
    ...
    and t.status in ( :status )
    and coalesce(t."queueId", 0) in ( :queuesIdsUser )
    and (( :isShowAll = 'N' and  (
      (:isExistsQueueTenant = 'S')
  ...
  `;
    ...
    and (( :isUnread = 'S'  and t."unreadMessages" > 0) OR (:isUnread = 'N'))
    and ((:isNotAssigned = 'S' and t."userId" is null) OR (:isNotAssigned = 'N'))
    and ((:isNotViewAssignedTickets = 'S' and ((t."userId" = :userId and t.status  in ( :status )) or (t."userId" is null and t.status  in ( :status )))) OR (:isNotViewAssignedTickets = 'N'))
    and ((:isSearchParam = 'S' and ( /*exists (
      select 1 from "Messages" m where m."ticketId" = t.id
      and upper(m.body) like upper(:searchParam)
      ) or */ (t.id::varchar like :searchParam) or (exists (select 1 from "Contacts" c where c.id = t."contactId" and (upper(c."name") like upper(:searchParam) or c."number" like :searchParam)))) OR (:isSearchParam = 'N'))
