  ...
  io.on("connection", socket => {
    ...
    if (tenantId) {
      ...
      socket.on(`${tenantId}:userVerified`, async (user) => {
        const findedUser = await User.findByPk(user.userId)
        await findedUser.update({ isOnline: true })
        socket.emit(`${user.tenantId}:updateStatusUser`, findedUser)
      });
