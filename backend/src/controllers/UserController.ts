...
import ListUserChatInterno from "../services/UserServices/ListUserChatInterno";
import UsersGroups from "../models/UsersGroups";
import ListGroupsByUserId from "../services/GroupServices/ListGroupsByUserId";
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (users.length >= tenant?.maxUsers!) {
    throw new AppError("ERR_USER_LIMIT_USER_CREATION", 400);
  } else if (req.url === "/signup" && (await CheckSettingsHelper("userCreation")) === "disabled") {
...
export const chatInterno = async (req: Request, res: Response): Promise<Response> => {
  const { tenantId, id } = req.user;

  const users = await ListUserChatInterno(Number(id), Number(tenantId));

  return res.json({ users });
};


export const showGroups = async (req: Request, res: Response): Promise<Response> => {
  const { userId } = req.params;

  const grupos = await ListGroupsByUserId(Number(userId));
