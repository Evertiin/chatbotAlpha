...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (findedTicket && !!findedTicket.userId && findedTicket.userId != userId && NotViewAssignedTickets == 'S') {
    throw new AppError("Ticket já atribuido para outro usuário");
  }

  if (findedTicket) {
    return res.status(200).json(findedTicket);
  }
