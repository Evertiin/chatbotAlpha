          ...
          <div class="q-pa-md">
            <div class="q-layout q-flex">
              <span class="text-h6 text-weight-medium">
                🎉 Parabéns! Foi lançada uma nova versão da plataforma.
              </span>
            </div>
            <div class="q-layout q-flex q-justify-end">
...
export default {
  ...
  methods: {
    async checkAndShowReleaseNotesModal() {
      const usuario = JSON.parse(localStorage.getItem('usuario')) || {}
      const modalVersao = usuario.modalVersao

      if (modalVersao) {
        try {
          const response = await fetch('/releaseNotes.json')
          const releaseData = await response.json()
          this.releaseNotes = releaseData.notas
          this.dialogVisible = true
        } catch (error) {
          console.error('Erro ao obter dados das notas de versão: ', error)
        }
      }
    },
  ...
  beforeMount() {
    ...
    const modalVersao = usuario.modalVersao
    if (modalVersao) {
      this.checkAndShowReleaseNotesModal()
    }
    const theme = {
      mode,
      palette: 'palette1',
