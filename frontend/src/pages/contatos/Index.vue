<template>
  <div>
    <q-table class="my-sticky-dynamic" title="Contatos" :id="`tabela-contatos-${isChatContact ? 'atendimento' : ''}`"
      :data="contacts" :columns="columns" :loading="loading" row-key="id" virtual-scroll :virtual-scroll-item-size="48"
      :virtual-scroll-sticky-size-start="48" :pagination.sync="pagination" :rows-per-page-options="[0]"
      @virtual-scroll="onScroll" :bordered="isChatContact" :square="isChatContact" :flat="isChatContact"
      :separator="isChatContact ? 'vertical' : 'horizontal'" :class="{
      ...
      <template v-slot:top>
        ...
        }" style="width: 300px" filled dense debounce="500" v-model="filter" clearable placeholder="Localize"
          @input="filtrarContato">
          <template v-slot:prepend>
            <q-icon name="search" />
          </template>
        </q-input>
        <q-btn-dropdown split class="q-ml-md" color="teal" label="Opções" @click="onMainClick">
          <q-list class="q-ml-md" style="height: 80px; align-items: center;">
            <q-btn color="warning" icon="autorenew" label="Sincronizar" @click="confirmarSincronizarContatos('whatsapp')" />
            <q-btn color="red" icon="delete" label="Excluir contatos" />
            <q-btn color="black" icon="get_app" label="Importar" v-if="!isChatContact" @click="modalImportarContatos = true" />
            <q-btn color="grey-8" icon="upgrade" v-if="!isChatContact" label="Exportar" @click="handleExportContacts" />
            <q-btn color="primary" icon="add" label="Adicionar" @click="selectedContactId = null; modalContato = true" />
          </q-list>
        </q-btn-dropdown>
      </template>
      <template v-slot:body-cell-profilePicUrl="props">
        <q-td>
          <q-avatar style="border: 1px solid #9e9e9ea1 !important">
            ...
            <q-img :src="props.value" style="max-width: 150px">
      ...
      <template v-slot:body-cell-acoes="props">
        <q-td class="text-center">
          ...
          <q-btn flat round icon="img:instagram-logo.png" @click="handleSaveTicket(props.row, 'instagram')"
            v-if="props.row.instagramPK" />
          <q-btn flat round icon="img:telegram-logo.png" @click="handleSaveTicket(props.row, 'telegram')"
    ...
    <ContatoModal :contactId="selectedContactId" :modalContato.sync="modalContato"
      @contatoModal:contato-editado="UPDATE_CONTACTS" @contatoModal:contato-criado="UPDATE_CONTACTS" />
      ...
      <q-card style="width: 400px;">
        ...
        <q-card-section>
          <q-file ref="PickerFileMessage" id="PickerFileMessage" bg-color="blue-grey-1" outlined dense use-chips
            accept=".csv" v-model="file" label="Importar contatos" hint="O arquivo deve conter as colunas Nome e Numero">
            <template v-slot:prepend>
        ...
        <q-card-actions align="right">
          <q-btn class="q-ml-md" color="negative" label="Cancelar" v-close-popup
            @click="isImportCSV = false; modalImportarContatos = false" />

          <q-btn class="q-ml-md" color="positive" label="Confirmar" @click="handleImportCSV" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
