  ...
  <div>
    <q-header class="bg-white text-grey-10 no-border-radius">
      <q-toolbar style="min-height: 60px; height: 60px;" class="no-border-radius q-pa-none ">
        ...
        <q-item clickable v-ripple class="q-ma-none q-pa-none full" style="min-height: 60px; height: 60px; width: 300px;"
          ...
          <q-item-section id="InfoCabecalhoChat">
            <q-item-label class="text-bold">
              ...
              <q-skeleton v-if="!Value(cticket.contact, 'name')" animation="none" style="width: 230px" />
            </q-item-label>
            <q-item-label caption lines="1" style="margin-top: 2px !important;"
              :style="$q.screen.width < 500 ? 'max-width: 170px' : ''">
              <span v-if="Value(cticket.user, 'name')"> Atribuido à: {{ Value(cticket.user, 'name') }} </span>
              <q-skeleton v-else type="text" class="text-caption" animation="none" style="width: 150px" />
            </q-item-label>
            <q-item-label lines="1" style="margin-top: 0px !important;">
              <span v-if="Value(cticket.contact, 'name')" class="" style="font-size: 11px"> Ticket: {{ cticket.id
              }}</span>
            </q-item-label>
          </q-item-section>
        </q-item>
        ...
        <div class="q-gutter-xs q-pr-sm" v-if="Value(cticket.contact, 'name')">
          <template v-if="!$q.screen.xs">
            <q-btn @click="$emit('updateTicket:reabrir')" flat autofocus icon="mdi-reload" color="cyan"
              class="bg-padrao btn-rounded" :disable="cticket.status == 'open' || cticket.whatsapp.isDeleted">
            ...
            <q-btn @click="$emit('abrir:modalAgendamentoMensagem')" flat icon="mdi-message-text-clock-outline"
              color="amber-9" class="bg-padrao btn-rounded"
              :disable="cticket.status == 'closed' || cticket.whatsapp.isDeleted">
            ...
            <q-btn @click="$emit('updateTicket:retornar')" flat icon="mdi-replay" color="negative"
              class="bg-padrao btn-rounded" :disable="cticket.status == 'closed' || cticket.whatsapp.isDeleted">
            ...
            <q-btn @click="$emit('updateTicket:resolver')" color="positive" flat class="bg-padrao btn-rounded"
              icon="mdi-comment-check" :disable="cticket.status == 'closed'">
          ...
          <template v-else>
            <q-fab :disable="cticket.status == 'closed'" color="primary" flat dense class="bg-padrao text-bold "
              icon="keyboard_arrow_left" direction="down" padding="5px" label="Ações" :class="{
                'bg-black': $q.dark.isActive

              }">
              <q-fab-action @click="$emit('updateTicket:resolver')" color="positive" :disable="cticket.status == 'closed'"
                flat class="bg-padrao q-pa-xs " icon="mdi-comment-check" :class="{
                  'bg-black': $q.dark.isActive
              ...
              <q-fab-action @click="$emit('updateTicket:retornar')" flat icon="mdi-replay" color="negative"
                :disable="cticket.status == 'closed' || cticket.whatsapp.isDeleted" class="bg-padrao q-pa-xs " :class="{
                  'bg-black': $q.dark.isActive

                }">
                <q-tooltip content-class="bg-negative text-bold">
                  Retornar Atendimentos para a Fila
                </q-tooltip>
              </q-fab-action>

              <q-fab-action @click="listarFilas" flat color="primary" class="bg-padrao q-pa-xs "
                :disable="cticket.status == 'closed' || cticket.whatsapp.isDeleted" :class="{
      ...
      <q-card class="q-pa-md" style="width: 500px">
        ...
        <q-card-section>
          <q-select square outlined v-model="filaSelecionada" :options="filas" emit-value map-options option-value="id"
            option-label="queue" label="Fila de destino" />
        </q-card-section>
        <q-card-section>
        ...
        <q-card-actions align="right">
          <q-btn flat label="Sair" color="negative" v-close-popup class="q-mr-lg" />
  ...
  methods: {
    ...
    async confirmarTransferenciaTicket() {
      ...
      if (this.ticketFocado.userId === this.usuarioSelecionado && this.ticketFocado.userId != null) {
        this.$q.notify({
          type: 'info',
          message: 'Atendimento já pertece ao usuário selecionado.',
          progress: true,
          actions: [{
            icon: 'close',
            round: true,
      ...
      if (this.ticketFocado.userId === userId && userId === this.usuarioSelecionado) {
        this.$q.notify({
          type: 'info',
          message: 'Atendimento já pertece ao seu usuário',
          progress: true,
          actions: [{
            icon: 'close',
            round: true,
      ...
      if (this.ticketFocado.queueId === this.filaSelecionada && this.ticketFocado.userId === this.usuarioSelecionado) {
        this.$q.notify({
          type: 'info',
          message: 'Atendimento já pertece a esta fila e usuário',
          progress: true,
          actions: [{
            icon: 'close',
            round: true,
