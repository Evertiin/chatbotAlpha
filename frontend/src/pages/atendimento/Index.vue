<template>
  <div class="WAL position-relative bg-grey-3" :style="style">
    <q-layout class="WAL__layout shadow-3" container view="lHr LpR lFr">
      ...
      <q-drawer v-model="drawerTickets" @hide="drawerTickets = false" show-if-above :overlay="$q.screen.lt.md" persistent
        :breakpoint="769" bordered :width="$q.screen.lt.md ? $q.screen.width : 380"
        ...
        <q-toolbar v-show="toolbarSearch" class="row q-gutter-sm q-py-sm items-center">
              ...
              <div class="row q-pa-sm" style="min-width: 350px; max-width: 350px">
                <div class="q-ma-sm">
                  ...
                  <q-toggle v-if="profile === 'admin'" class="q-ml-lg" v-model="pesquisaTickets.showAll"
                    label="(Admin) - Visualizar Todos Atendimentos Sem Filtro" :class="{ 'q-mb-lg': pesquisaTickets.showAll }"
                  ...
                  <div v-if="!pesquisaTickets.showAll">
                    ...
                    <q-toggle v-model="pesquisaTickets.withUnreadMessages" label="Somente Atendimentos com mensagens não lidas"
                      @input="debounce(BuscarTicketFiltro(), 700)" />
                    <q-toggle v-model="pesquisaTickets.isNotAssignedUser"
                      label="Somente Atendimentos não atribuidos (sem usuário definido)"
                      @input="debounce(BuscarTicketFiltro(), 700)" />
                  </div>
          ...
          <q-input v-model="pesquisaTickets.searchParam" dense outlined rounded type="search" class="col-grow"
            :debounce="700" @input="BuscarTicketFiltro()">
        ...
        >
          <q-tab
            ...
            :label="'Abertos'"
            :active-color="cFiltroSelecionado === 'abertos' ? 'deep-orange-9' : 'primary'"
            :class="{ 'tab-selected': cFiltroSelecionado === 'abertos' }"
            @click="setFiltro('abertos')"
          >
          </q-tab>
          ...
          <q-tab
            ...
            :label="'Finalizados'"
            :active-color="cFiltroSelecionado === 'finalizados' ? 'deep-orange-9' : 'primary'"
            :class="{ 'tab-selected': cFiltroSelecionado === 'finalizados' }"
            @click="setFiltro('finalizados')"
          >
          </q-tab>
        </q-tabs>
        <!-- Botão "Em andamento" -->
        ...
        >
          <q-tab
            v-if="mostrarEmAndamento"
            name="emAndamento"
            icon="mdi-message-outline"
            :label="'Em andamento'"
            :active-color="cFiltroSelecionado === 'emAndamento' ? 'deep-orange-9' : 'primary'"
            @click="setFiltro('emAndamento')"
          >
          </q-tab>
          ...
          <q-tab
            ...
            :label="'Na fila'"
            :active-color="cFiltroSelecionado === 'naFila' ? 'deep-orange-9' : 'primary'"
            @click="setFiltro('naFila')"
          >
          </q-tab>
        </q-tabs>

        <q-scroll-area ref="scrollAreaTickets" style="height: calc(100% - 180px)" @scroll="onScroll">
          <!-- <q-separator /> -->
          <ItemTicket v-for="(ticket, index) in displayedTickets" :key="index" :ticket="ticket" :filas="filas" />
          <div v-if="loading">
            <div class="row justify-center q-my-md">
              <q-spinner color="$q.dark.isActive ? 'grey-3' : 'black'" size="3em" :thickness="3" />
            </div>
            <div color="$q.dark.isActive ? 'grey-3' : 'black'" class="row col justify-center q-my-sm">
              Carregando...
            </div>
          </div>
        </q-scroll-area>
        ...
        <div class="absolute-bottom row justify-between" style="height: 50px">
          <q-toggle size="xl" keep-color dense class="text-bold q-ml-md flex flex-block"
          ...
          <div class="flex flex-inline q-pt-xs">
            <q-scroll-area horizontal style="heigth: 40px; width: 300px; background-color: white;">
              <template v-for="item in whatsapps">
                <q-btn rounded flat dense size="18px" :key="item.id" class="q-mx-xs q-pa-none"
                  :style="`opacity: ${item.status === 'CONNECTED' ? 1 : 0.2}`" :icon="`img:${item.type}-logo.png`">
              ...
              <q-btn v-if="showLoadMoreButton" @click="loadMoreTickets" label="Mostrar Mais" class="align-right" />
            </q-scroll-area>
          </div>
        </div>
      </q-drawer>
      ...
      <q-drawer v-if="!cRouteContatos && ticketFocado.id" v-model="drawerContact" show-if-above bordered side="right"
          ...
          <div class="q-pa-sm">
            <q-card class="bg-white btn-rounded" style="width: 100%" bordered flat>
              <q-card-section class="text-center">
                <q-avatar style="border: 1px solid #9e9e9ea1 !important; width: 100px; height: 100px">
                  <q-icon name="mdi-account" style="width: 100px; height: 100px" size="6em" color="grey-5"
                ...
                <div class="text-caption q-mt-sm" style="font-size: 14px" id="number"
                ...
                <q-btn color="primary" class="q-mt-sm bg-padrao btn-rounded" flat icon="edit" label="Editar Contato"
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat>
              <q-card-section class="text-bold q-pa-sm ">
                <q-btn flat class="bg-padrao btn-rounded" :color="!$q.dark.isActive ? 'grey-9' : 'white'" label="Logs"
                  icon="mdi-timeline-text-outline" @click="abrirModalLogs" />
              </q-card-section>
            </q-card>
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat
              ...
              <q-card-section class="q-pa-none">
                <q-select square borderless :value="ticketFocado.contact.tags" multiple :options="etiquetas" use-chips
                  option-value="id" option-label="tag" emit-value map-options dropdown-icon="add" @input="tagSelecionada">
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat
              ...
              <q-card-section class="q-pa-none">
                <q-select square borderless :value="ticketFocado.contact.wallets" multiple :max-values="1"
                  :options="usuarios" use-chips option-value="id" option-label="name" emit-value map-options
                  dropdown-icon="add" @input="carteiraDefinida">
                  <template v-slot:option="{ itemProps, itemEvents, opt, selected, toggleOption }">
                    <q-item v-bind="itemProps" v-on="itemEvents">
                      <q-item-section>
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat
              ...
              <q-card-section class="q-pa-none">
                <template v-if="ticketFocado.scheduledMessages">
                  <q-list>
                    <q-item v-for="(message, idx) in ticketFocado.scheduledMessages" :key="idx" clickable>
                      <q-item-section>
                        <q-item-label caption>
                          <b>Agendado para:</b> {{ $formatarData(message.scheduleDate, 'dd/MM/yyyy HH:mm') }}
                          <q-btn flat round dense icon="mdi-trash-can-outline" class="absolute-top-right q-mr-sm"
                            size="sm" @click="deletarMensagem(message)" />
                        </q-item-label>
                        <q-item-label caption lines="2"> <b>Msg:</b> {{ message.mediaName || message.body }}
                        </q-item-label>
                      </q-item-section>
                      <q-tooltip :delay="500">
        ...
        <q-card style="width: 400px">
          ...
          <q-card-section class="">
            <q-scroll-area style="height: calc(100vh - 200px);" class="full-width">
              <q-timeline color="black" style="width: 360px" class="q-pl-sm "
...
import { ListarFilasPorUser, ListarFilas } from 'src/service/filas'
...
export default {
  ...
  data() {
    return {
      ...
      logsTicket: [],
      mostrarEmAndamento: true,
      mostrarNaFila: true,
      displayedTickets: [], // Lista de tickets a serem exibidos
      batchSize: 20, // Tamanho do lote para carregar de cada vez
  ...
  methods: {
    loadMoreTickets() {
      const startIndex = this.displayedTickets.length
      const endIndex = startIndex + this.batchSize
      this.displayedTickets = this.displayedTickets.concat(this.tickets.slice(startIndex, endIndex))

      // Verifique se todos os tickets foram exibidos
    ...
    setFiltro(filtro) {
      if (filtro === 'abertos') {
        ...
        this.mostrarEmAndamento = true
        this.mostrarNaFila = true
      } else if (filtro === 'finalizados') {
        this.pesquisaTickets.status = ['closed']
        this.mostrarEmAndamento = false
        this.mostrarNaFila = false
      } else if (filtro === 'emAndamento') {
        this.pesquisaTickets.status = ['open']
        this.mostrarEmAndamento = true
        this.mostrarNaFila = true
      } else if (filtro === 'naFila') {
        this.pesquisaTickets.status = ['pending']
        this.mostrarEmAndamento = true
        this.mostrarNaFila = true
      } else {
        this.pesquisaTickets.status = [filtro]
        this.mostrarEmAndamento = false
        this.mostrarNaFila = false
      }
    ...
    async consultarTickets(paramsInit = {}, action) {
      const params = {
      ...
      try {
        ...
        this.$store.commit('LOAD_TICKETS', { data: data.tickets, action })
        console.log(data.tickets, action)
    ...
    async BuscarTicketFiltro() {
      ...
      if (this.pesquisaTickets.showAll) {
        this.pesquisaTickets = {
          ...this.pesquisaTickets,
      ...
      console.log('aqui', this.pesquisaTickets)
      this.pesquisaTickets = {
        ...this.pesquisaTickets,
        pageNumber: 1
      }

      await this.consultarTickets(this.pesquisaTickets, 'create')
      this.loading = false
    ...
    async listarFilas() {
      ...
      if (usuario.profile == 'admin') {
        response = await ListarFilas()
      } else {
        response = await ListarFilasPorUser()
      }
      const { data } = response
      this.filas = data
      this.filas.push({ id: 0, queue: 'Sem Fila' })
  ...
  watch: {
    tickets(newTickets) {
      // Atualiza a lista de tickets exibidos quando a lista completa é alterada
      this.displayedTickets = newTickets.slice(0, this.batchSize)
      this.showLoadMoreButton = newTickets.length > this.batchSize
    }
  },
  async mounted() {
    this.loadMoreTickets() // Carregue os primeiros tickets ao montar o componente
    ...
    await this.consultarTickets({}, 'create')
