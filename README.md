...
```bash
git clone git@github.com:LeadPyTecnologia/chatbot-alisson
```
...
```bash
cd  ~
git clone git@github.com:ldurans/chatpro.io.git
```

Na pasta backend, crie o arquivo .env:

```bash
cp chatpro/backend/.env.example chatpro/backend/.env
nano chatpro/backend/.env
```
...
pm2 start dist/server.js --name chatpro-backend
```
...
```bash
sudo nano /etc/nginx/sites-available/chatpro-backend
```
...
```bash
sudo nano /etc/nginx/sites-available/chatpro-frontend
```
...
server {
  ...
  root /home/user/chatpro/frontend/dist/pwa; # caminho da pasta dist/pwa
...
```bash
sudo ln -s /etc/nginx/sites-available/chatpro-backend /etc/nginx/sites-enabled
sudo ln -s /etc/nginx/sites-available/chatpro-frontend /etc/nginx/sites-enabled
```
...
chatpro é um trabalho em progresso e estamos frequentemente adicionando novas funcionalidades e correções de bugs.
...
```bash
#!/bin/bash
echo  "Atualizando chatpro, aguarde..."
cd  ~
cd chatpro
git pull
cd backend
